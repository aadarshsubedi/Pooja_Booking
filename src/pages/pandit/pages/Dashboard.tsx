...
const Dashboard: React.FC = () => {
  ...
  return (
    <div>
      <h3>Welcome, {data.pandit_username}</h3>

      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
        <div className="card">Upcoming: {data.upcoming}</div>
        <div className="card">Pending: {data.pending}</div>
        <div className="card">Confirmed: {data.confirmed}</div>
        <div className="card">Completed: {data.completed}</div>
        <div className="card">Cancelled: {data.cancelled}</div>
        <div className="card">Earnings: Rs. {data.earnings}</div>
      </div>
    </div>
  );
};
